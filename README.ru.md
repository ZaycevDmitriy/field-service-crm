...
Версионирование и release notes автоматизированы через [semantic-release](https://semantic-release.gitbook.io/)
по [Conventional Commits](https://www.conventionalcommits.org/): версия — *выход*, вычисляемый из
коммитов с прошлого релиза, а не задаётся вручную.

Момент релиза контролируется вручную — открыть pull request из `main` в ветку `release`. Его мерж
запускает [релизный пайплайн](.github/workflows/release-android.yml) на GitHub Actions:

1. прогон quality-гейта (lint, typecheck, format, тесты),
2. расчёт следующей версии по истории коммитов,
3. сборка Android APK на раннере (нативный Gradle, без EAS-кредитов) с этой версией,
4. публикация [GitHub Release](../../releases) с APK и его fingerprint (`runtimeVersion`),
5. обновление [`CHANGELOG.md`](CHANGELOG.md) и back-merge `release` → `main`.
...
| Что меняли | Как доставляется |
|------------|------------------|
| Нативный код или пакеты (SDK, разрешения, нативные модули, иконки) | Новый **релиз APK** через пайплайн выше |
| Только JS / TS и ассеты | **EAS Update** (OTA) в канал `production` |

OTA защищён runtime version по политике `fingerprint`: JS-обновление доедет только до сборки с
совпадающим нативным отпечатком (см. [Демо EAS Update](#демо-eas-update)).
