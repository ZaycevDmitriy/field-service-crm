...
> **Для рекрутеров (TL;DR)**
> - **Что это:** offline-first мобильная CRM для выездных сервисных работников — заявки, статусы,
>   фотоотчёты, маршруты, напоминания о визите.
> - **Стек:** TypeScript (`strict`), React Native + Expo SDK 56, Expo Router, Zustand, SQLite,
>   EAS Build / EAS Update (OTA), GitHub Actions + semantic-release.
> - **Что смотреть:** [Архитектура](#архитектура) (Feature-Sliced Design),
>   [Релизы](#релизы) (автоматизированный CI/CD с выбором доставки APK / OTA по fingerprint),
>   [Скриншоты](#скриншоты).
> - **Попробовать:** [скачать Android APK](#скачать-android-apk) — сборка не требуется.

**Onsite** (имя репозитория — `field-service-crm`, по предметной области) — мобильное mini-CRM для
выездных сервисных работников (установка роутеров, диагностика линий, ремонт кабеля). Это
портфолио-проект на Expo и React Native с акцентом на offline-first, нативные API устройства и
чистую feature-ориентированную архитектуру.
...
Onsite имитирует реальный класс production-софта — field-service / workforce-приложения, которыми
пользуются курьеры, инженеры телекома, монтажники и сервисные бригады (внутренние инструменты
провайдеров, коммунальных и курьерских компаний). Ограничения здесь настоящие:

- работник трудится в подвалах и промзонах без связи, поэтому **offline — норма**, а не edge case;
- **камера, GPS и уведомления — часть рабочего процесса**, а не дополнение;
- приложение должно **обновляться «в поле»** (OTA), без полной переустановки.

Проект — портфолио, но задачи, которые он решает, взяты из production-практики.
...
Релизный пайплайн прикладывает APK к [GitHub Release](../../releases), когда меняется нативный
слой; JS-only релизы доставляются как OTA-обновления поверх последнего APK.

**[⬇ Скачать Android APK](https://github.com/ZaycevDmitriy/field-service-crm/releases/download/v1.2.3/onsite-v1.2.3.apk)**
(или взять `.apk` из последнего [релиза](../../releases), где он есть). При запуске приложение само
подтянет свежий JS через EAS Update — это одновременно живое демо OTA-флоу.

> APK подписан собственным release-keystore проекта, поэтому Android попросит разрешить установку
> из этого источника и может показать предупреждение «приложение не проверено» — для сборки вне
> стора это ожидаемо.

### Локальный запуск

Хостинг-демо, кроме APK, нет — запускайте приложение локально на development build (см.
...
- **Фотоотчёт** — прикрепление фото к заявке с камеры или из галереи за экраном разрешения;
  полноэкранный просмотр и удаление фото (удаление доступно только в статусе «В работе»).
...
| `production` | стора-сборка (версия из релизного пайплайна) | store | production |
...
Проект использует gitflow **develop / main**: повседневная разработка идёт в `develop`, релиз — один
pull request из `develop` в `main`. Версионирование и release notes автоматизированы через
[semantic-release](https://semantic-release.gitbook.io/) по
[Conventional Commits](https://www.conventionalcommits.org/): версия — *выход*, вычисляемый из
коммитов с прошлого релиза, а не задаётся вручную.

Момент релиза контролируется вручную — открыть pull request из `develop` в `main` и смержить
**merge-коммитом** (`--no-ff`, *не* squash — иначе гранулярные `feat:` / `fix:` схлопнутся и расчёт
версии собьётся). Push в `main` запускает [релизный пайплайн](.github/workflows/release.yml) на
GitHub Actions:

1. прогон quality-гейта (lint, typecheck, format, тесты),
2. расчёт следующей версии по истории коммитов, обновление [`CHANGELOG.md`](CHANGELOG.md), тег
   `v<версия>` и [GitHub Release](../../releases) с заметками,
3. автоматический выбор канала доставки по нативному fingerprint (см. ниже),
4. back-merge `main` → `develop`, чтобы ветки не расходились.

Тип коммита определяет bump: `feat:` → minor, `fix:` / `perf:` → patch, `feat!:` или футер
`BREAKING CHANGE:` → major; `chore:` / `docs:` / `ci:` релиза не дают. Тип коммита английский,
описание русское.

### Контуры доставки

Один релиз сам выбирает канал, сравнивая текущий нативный fingerprint с последним релизом:

| Что меняли | Как доставляется |
|------------|------------------|
| Нативный код или пакеты (SDK, разрешения, нативные модули, иконки) — fingerprint изменился | Новый **APK**, собранный на раннере (нативный Gradle) и приложенный к GitHub Release |
| Только JS / TS и ассеты — fingerprint не изменился | **EAS Update** (OTA) в канал `production` |

Безопасность обеспечивает runtime version по политике `fingerprint`: JS-обновление доедет только до
сборки с совпадающим нативным отпечатком (см. [Демо EAS Update](#демо-eas-update)).
